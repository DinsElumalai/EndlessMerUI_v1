...
@Component({
  selector: 'app-list-ppon',
  templateUrl: './list-ppon.component.html',
  styleUrls: ['./list-ppon.component.scss']
})
export class ListPponComponent implements OnInit {
  ...
  reloadData()
  {
    this.adminService.getList(ApiUrls.transactionByReqCodeApi + "/PPON").subscribe(
      data =>  {
                  this.noteSlips = data;
     ...
     if(this.authService.getUserRoles().includes('rl_ppon_delete'))
          this.isDelete = true;
          
          if(this.authService.getUserRoles().includes('rl_ppon_verify'))
          this.isVerify = true;

        if(this.authService.getUserRoles().includes('rl_ppon_unverify'))
