...
@Component({
  selector: 'app-list-pdrn',
  templateUrl: './list-pdrn.component.html',
  styleUrls: ['./list-pdrn.component.scss']
})
export class ListPdrnComponent implements OnInit {
  ...
  reloadData()
  {
    this.adminService.getList(ApiUrls.transactionByReqCodeApi + "/PDRN").subscribe(
      data =>  {
                  this.noteSlips = data;
        ...
        if(this.authService.getUserRoles().includes('rl_pdrn_delete'))
          this.isDelete = true;
          
          if(this.authService.getUserRoles().includes('rl_pdrn_verify'))
          this.isVerify = true;

        if(this.authService.getUserRoles().includes('rl_pdrn_unverify'))
